const DEFAULT_FROM_EMAIL = process.env.RESEND_FROM_EMAIL || process.env.EMAIL_FROM_EMAIL || 'no-reply@findfirst.org';
...
function getEmailConfigStatus() {
    ...
    return {
        ...
        hasFromEmail: Boolean(String(process.env.RESEND_FROM_EMAIL || process.env.EMAIL_FROM || '').trim())
    };
}

function getEmailConfigErrorMessage() {
    const status = getEmailConfigStatus();
    if (!status.hasApiKey) return 'Resend is not configured. Missing RESEND_API_KEY in .env.';
    return 'Resend sender is using the default from address.';
}
