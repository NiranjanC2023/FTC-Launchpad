...
function buildEmailIdempotencyKey(payload) {
    const canonicalPayload = JSON.stringify({
        from: String(payload.from || ''),
        to: normalizeRecipientList(payload.to).map(value => value.toLowerCase()).sort(),
        subject: String(payload.subject || ''),
        html: String(payload.html || ''),
        text: String(payload.text || ''),
        replyTo: String(payload.reply_to || payload.replyTo || '')
    });
    const digest = crypto.createHash('sha256').update(canonicalPayload).digest('hex');
    return `first-start/${digest}`;
}

async function sendViaResend(apiKey, payload, sender, recipients) {
    const idempotencyKey = buildEmailIdempotencyKey(payload);
    const response = await fetch('https://api.resend.com/emails', {
        ...
        headers: {
            ...
            'User-Agent': 'FIRST-Start-Website/1.0'
        },
    ...
    try {
        ...
        if (parsed && parsed.message) errorMessage = String(parsed.message);
    } catch (error) {}
    ...
    <div style="padding:32px 16px;">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #dbe8df;border-radius:20px;overflow:hidden;box-shadow:0 18px 50px rgba(15, 48, 35, 0.10);">
        <div style="padding:28px 32px 20px;background:linear-gradient(135deg,#0f3922,#145437);color:#fff;">
          <div style="display:inline-block;padding:8px 12px;border-radius:999px;background:rgba(255,255,255,0.12);font-size:12px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;">Find FIRST</div>
        ...
        <div style="padding:18px 32px 28px;border-top:1px solid #e2ece6;background:#fbfdfb;color:#6a7e73;font-size:13px;line-height:1.6;">
          <div style="font-weight:700;color:#355145;margin-bottom:4px;">Find FIRST Support</div>
          <div>${safeFooter}</div>
        </div>
      </div>
    </div>
...
async function sendTransactionalEmail(mailOptions = {}) {
    ...
    const apiKey = getResendApiKey();
    if (apiKey) {
        // Never fall through to a second provider after an ambiguous API error:
        // the first provider may already have accepted the message.
        return sendViaResend(apiKey, payload, sender, recipients);
    }
    ...
    if (smtpTransport) {
        const info = await smtpTransport.sendMail({
            ...
            headers: {
                'Resend-Idempotency-Key': buildEmailIdempotencyKey(payload)
            }
        });
        return {
